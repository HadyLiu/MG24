...
void entry_Init(void) {
  ...
  const bool mainLightActive = (LightDecisionCenter::Instance().GetCurrentBrightness() > 0U) ||
                               LightEffectEngine::Instance().IsAnyChannelActive();
  LowPowerCoordinator::Instance().OnMainLightActivity(mainLightActive);
  s_mainLightMatterFullSpeedHeld = (LightDecisionCenter::Instance().GetCurrentBrightness() > 0U);
  MatterBridge::Instance().SetMainLightIcdHold(s_mainLightMatterFullSpeedHeld);
  LowPowerCoordinator::Instance().OnUsbPowerActivity(BspPowerMonitor::Instance().GetUsbStatus() ==
...
void power_Wire(void) {
  ...
  LightEffectEngine::Instance().RegisterOutputActivityCallback([](bool isActive) {
    ...
    const bool holdActive = (LightDecisionCenter::Instance().GetCurrentBrightness() > 0U) || isActive;
  ...
  PowerServer::Instance().RegisterChargeStatusHandler([](const BatteryChargeSnapshot& snapshot) {
    ...
    const bool usbPresent = (BspPowerMonitor::Instance().GetUsbStatus() == UsbConnectionStatusEnum::UsbConnected);
    LowPowerCoordinator::Instance().OnUsbPowerActivity(usbPresent);
    const bool indicActive = (IndicatorServer::Instance().GetAppliedEffects() != ChargeIndicatorEffect::Off);
...
void button_Init(void) {
  ...
  ButtonService::Instance().RegisterKeyEventHandler([](KeyEventType event) {
    ...
    if ((event == KeyEventType::ShortPressCycleBrightness) || (event == KeyEventType::ShortPressOpenCommissioning) ||
        (event == KeyEventType::DoublePressCycleColor) || (event == KeyEventType::LongPressClearNetLighting) ||
    ...
    const bool isNetResetKey = (event == KeyEventType::LongPressClearNetLighting) ||
                               (event == KeyEventType::LongPressClearNet) || (event == KeyEventType::LongPressStopNet);
    if (!isNetResetKey) {
...
void Matter_Init(void) {
  ...
  MatterBridge::Instance().SetFirstCommissionPending(LightDecisionCenter::Instance().ShouldShowFirstCommissionBreath());

  MatterBridge::Instance().RegisterCommissioningUiCallback([](bool windowOpenedForFirstPair, bool commissioningDone) {
    ...
    if (windowOpenedForFirstPair && LightDecisionCenter::Instance().ShouldShowFirstCommissionBreath()) {
      IndicatorServer::Instance().OnFirstCommissionBreathStart();
    }
  });

  MatterBridge::Instance().RegisterCommissioningSessionCallback(
      [](bool sessionActive) { LowPowerCoordinator::Instance().SetCommissioningHold(sessionActive); });
  ...
  LightDecisionCenter::Instance().RegisterMatterReporter([](bool on, uint8_t brightness, const uint16_t* pWrgb) {
    ...
    if (pWrgb[0] > 0U) {
      ...
      LOG_MATTER("Color uplink CT: K=%u mireds=%u WRGB=%u,%u,%u,%u", static_cast<unsigned>(kelvin), mireds, pWrgb[0],
      ...
      LOG_MATTER("Color uplink HSV: H=%u S=%u WRGB=%u,%u,%u,%u", hsv.h, hsv.s, pWrgb[0], pWrgb[1], pWrgb[2], pWrgb[3]);
    ...
    switch (d.kind) {
      case MatterBridgeServer::DownlinkKind::kLightControl:
        ...
        LightDecisionCenter::Instance().ProcessMatterCommand(
            d.wrgb, d.brightness, d.brightness > 0U ? LightEffectOpId::LinearLerp : LightEffectOpId::Bezier40FadeOut);
        SyncMainLightMatterFullSpeedRaw();
        break;
