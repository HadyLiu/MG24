...
void MatterBridgeServer::EmitLightControlRaw() {
  ...
  if (m_cacheOn) {
    data.brightness = (m_cacheBrightness > 0U) ? m_cacheBrightness : m_lastOnBrightness;
  } else {
    data.brightness = 0U;
  }
...
void MatterBridgeServer::OnMatterDataReceived(MatterDownlinkUploadPayload mdc) {
  ...
  switch (mdc.element) {
    ...
    case MatterDataElement::kBrightness:
      ...
      {
        const uint8_t driverBrightness = LightDimmingSpec::MatterLevelToDriverBrightness(mdc.brightness);
