...
bool FindColorPaletteIndexRaw(const uint16_t* pWrgb, uint8_t* outIndex) {
  ...
  for (uint8_t i = 0U; i < kColorPaletteCount; ++i) {
    if ((pWrgb[0] == kColorPalette[i][0]) && (pWrgb[1] == kColorPalette[i][1]) && (pWrgb[2] == kColorPalette[i][2]) &&
        (pWrgb[3] == kColorPalette[i][3])) {
      *outIndex = i;
      return true;
    }
  }
...
void LightDecisionCenter::ProcessMatterCommand(const uint16_t* pWrgbBuffer, uint8_t brightness, LightEffectOpId opId) {
  ...
  const bool wasOn = (m_userTargetParam.brightness > 0U);
  const bool sameWrgb = (memcmp(m_userTargetParam.wrgb, pWrgbBuffer, sizeof(m_userTargetParam.wrgb)) == 0);
...
void LightDecisionCenter::MarkFirstCommissionDone() {
  ...
  m_userTargetParam.reserved =
      static_cast<uint8_t>(m_userTargetParam.reserved | kFirstCommissionDoneFlag | kFirstCommissionBreathDismissedFlag);
  SafeSaveToStorage();
...
void LightDecisionCenter::MarkFirstCommissionBreathDismissed() {
  ...
  m_userTargetParam.reserved = static_cast<uint8_t>(m_userTargetParam.reserved | kFirstCommissionBreathDismissedFlag);
  SafeSaveToStorage();
...
void LightDecisionCenter::StartIdentifySequence() {
  ...
  LightSequenceScheduler::SequenceStep kIdentifySteps[] = {
      {LightEffectProcessor::GetBlink, {0U, 0U, 0U, 0U}, kDefaultBrightness, BlinkTimingSpec::kNormalBlinkCycleMs, 0U}};
