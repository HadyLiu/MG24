...
void LowPowerCoordinator::Poll(uint16_t elapsedMs) {
  ...
  if (m_usbFallbackMs >= 1000U) {
    ...
    OnUsbPowerActivity(BspPowerMonitor::Instance().GetUsbStatus() == UsbConnectionStatusEnum::UsbConnected, true);
  }

  if (m_runState == RunState::IdleSettle) {
    m_idleElapsedMs = static_cast<uint16_t>(m_idleElapsedMs + elapsedMs);
...
void LowPowerCoordinator::SetRunStateRaw(RunState nextState) {
  ...
  LOG_LP("[LP] state %u -> %u hold=0x%02X", static_cast<unsigned>(m_runState), static_cast<unsigned>(nextState),
