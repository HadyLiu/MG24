...
static constexpr uint8_t kMinPhysicalDutyScale255 = static_cast<uint8_t>((255U * kMinPhysicalDutyPercent + 99U) / 100U);
...
inline uint8_t MapBrightnessToDutyScale(uint8_t brightness) {
  ...
  return static_cast<uint8_t>(kMinPhysicalDutyScale255 + (offset * dutySpan + (briSpan / 2U)) / briSpan);
}
}  // namespace LightDimmingSpec
