 ...
 * @date 2026-06-12
 * @layer Service
 * @note 由 10ms tick 驱动；混合特效与淡变共用同一状态机，禁止在 ISR 中调用。
 */
...
uint16_t LightEngine::calcFadeMsForBrightnessDelta(uint8_t from,
                                                   uint8_t to) const
{
  uint8_t delta = (from > to) ? static_cast<uint8_t>(from - to)
...
uint16_t LightEngine::resolveFadeMs(FadePolicy policy, uint8_t fromBrightness,
                                    uint8_t toBrightness,
                                    uint16_t fixedMs) const
{
  switch (policy)
  {
...
WrgbColor LightEngine::scaleColor(const WrgbColor& raw,
                                  uint8_t brightness) const
{
  WrgbColor target = {0, 0, 0, 0};
  uint32_t scale =
      204 +
      ((static_cast<uint32_t>(brightness <= 1 ? 1 : brightness) - 1) * 10035U) /
          254U;
  if (brightness <= 1)
    scale = 204;
    ...
    scale = 10240;
  target.w =
      static_cast<uint16_t>((static_cast<uint32_t>(raw.w) * scale) >> 14);
  target.r =
      static_cast<uint16_t>((static_cast<uint32_t>(raw.r) * scale) >> 14);
  target.g =
      static_cast<uint16_t>((static_cast<uint32_t>(raw.g) * scale) >> 14);
  target.b =
      static_cast<uint16_t>((static_cast<uint32_t>(raw.b) * scale) >> 14);
  return target;
}

void LightEngine::updateNormalState(bool on, uint8_t brightness,
                                    const WrgbColor& raw)
{
  m_state.is_on      = on;
...
void LightEngine::setOnOff(bool on, FadePolicy fade)
{
  ...
  startFadeToCurrent(
      resolveFadeMs(fade, prev, m_state.brightness, LED_FADE_COLOR_SWITCH_MS),
      fade);
}
...
void LightEngine::setBrightness(uint8_t level, FadePolicy fade)
{
  ...
  startFadeToCurrent(resolveFadeMs(fade, prev, level, LED_FADE_COLOR_SWITCH_MS),
                     fade);
}

void LightEngine::setColorFromCycleIndex(uint8_t cycleIndex, uint16_t fadeMs)
{
  m_state.color_cycle_index =
      (cycleIndex >= ColorLibrary::kCycleCount) ? 0 : cycleIndex;
  m_state.color_source = ColorSourceIndexTable;
  m_state.raw_color =
      ColorLibrary::instance().colorAtCycleIndex(m_state.color_cycle_index);
  startFadeToCurrent(fadeMs, FadePolicyFixedMs);
}

void LightEngine::setColorRaw(const WrgbColor& c, ColorSource src,
                              uint16_t fadeMs)
{
  m_state.raw_color    = c;
...
void LightEngine::startFadeInternal(uint16_t fadeMs, FadePolicy policy)
{
  ...
  m_state.target_color  = m_state.is_on
                              ? scaleColor(m_state.raw_color, m_state.brightness)
                              : WrgbColor{0, 0, 0, 0};
  m_state.fade_start_ms = SystemTime::nowMs();
...
void LightEngine::setBlink(uint8_t brightness, const WrgbColor& raw,
                           uint16_t periodMs, uint16_t count)
{
  if (m_state.low_battery_protected)
    return;
  m_state.effect_mode      = EffectModeBlink;
  m_state.effect_start_ms  = SystemTime::nowMs();
...
void LightEngine::setBreath(uint8_t brightness, const WrgbColor& raw,
                            uint16_t count)
{
  if (m_state.low_battery_protected)
    return;
...
void LightEngine::setHold(bool on, uint8_t brightness, const WrgbColor& raw,
                          uint16_t fadeMs)
{
...
void LightEngine::setFade(bool on, uint8_t brightness, const WrgbColor& raw,
                          uint16_t fadeMs)
{
  if (m_state.low_battery_protected)
    return;
  m_state.effect_mode = EffectModeFade;
  m_state.raw_color   = raw;
  m_state.brightness  = brightness;
  m_state.is_on       = on;
  m_state.start_color = m_state.cur_color;
  m_state.target_color =
      on ? scaleColor(raw, brightness) : WrgbColor{0, 0, 0, 0};
  m_state.fade_start_ms = SystemTime::nowMs();
  m_state.fade_time_ms  = (fadeMs == 0) ? 1 : fadeMs;
  m_state.fade_policy   = FadePolicyFixedMs;
  m_state.fading        = true;
}
...
void LightEngine::executeMixedIndex(uint8_t index)
{
  ...
  case EffectModeHold:
    ...
    break;
  case EffectModeFade:
    setFade(p->is_on, p->brightness, p->raw_color, p->fade_ms);
    break;
  default:
...
void LightEngine::mixedEffectsService()
{
  ...
  if (m_state.mix_effect_index >= m_state.mix_effect_end)
  {
    ...
    else
    {
      ...
      m_state.target_color =
          m_state.is_on ? scaleColor(m_state.raw_color, m_state.brightness)
                        : WrgbColor{0, 0, 0, 0};
      m_state.fade_start_ms = SystemTime::nowMs();
...
void LightEngine::runMixedSequence(const MixedEffectStep* steps,
                                   uint8_t stepCount, MixedEffectCallback cb)
{
  if (m_state.low_battery_protected || stepCount == 0 || stepCount > 5)
    return;
  LOG_LIGHT("[LightEngine] mixed sequence steps=%u", stepCount);
...
/** @brief 10ms 主 tick：驱动淡变/特效 FSM 并刷新 PWM 输出
 *  @return 无
 *  @note 声明见 LightEngine.h
 */
void LightEngine::tick10ms()
{
  ...
  do
  {
    ...
    {
      if (m_state.effect_count > 0 &&
          elapsed >= (static_cast<uint32_t>(m_state.effect_count) *
                      m_state.effect_period_ms))
      {
        stepDone = true;
        break;
      }
      ...
      m_state.cur_color =
          (tick < (m_state.effect_period_ms >> 1))
              ? scaleColor(m_state.raw_color, m_state.brightness)
              : WrgbColor{0, 0, 0, 0};
      ledChange = true;
      break;
    ...
    if (m_state.effect_mode == EffectModeBreath)
    {
      ...
      m_state.cur_color.w =
          static_cast<uint16_t>((static_cast<uint32_t>(base.w) * factor) >> 12);
      m_state.cur_color.r =
          static_cast<uint16_t>((static_cast<uint32_t>(base.r) * factor) >> 12);
      m_state.cur_color.g =
          static_cast<uint16_t>((static_cast<uint32_t>(base.g) * factor) >> 12);
      m_state.cur_color.b =
          static_cast<uint16_t>((static_cast<uint32_t>(base.b) * factor) >> 12);
      ledChange = true;
      break;
    }

    if (m_state.effect_mode == EffectModeFade)
    {
      ...
      m_state.cur_color.w =
          lerpLin(m_state.start_color.w, m_state.target_color.w);
      m_state.cur_color.r =
          lerpLin(m_state.start_color.r, m_state.target_color.r);
      m_state.cur_color.g =
          lerpLin(m_state.start_color.g, m_state.target_color.g);
      m_state.cur_color.b =
          lerpLin(m_state.start_color.b, m_state.target_color.b);
      ledChange = true;
      break;
    }
    ...
    if (m_state.effect_mode == EffectModeNone && m_state.fading)
    {
      ...
      else if (m_state.fade_policy == FadePolicyEaseOutQuad)
      {
        ...
        auto lerp  = [&](uint16_t a, uint16_t b) {
          return static_cast<uint16_t>(a + ((static_cast<int32_t>(b) - a) * t) /
                                               4096);
        };
        m_state.cur_color.w =
            lerp(m_state.start_color.w, m_state.target_color.w);
        m_state.cur_color.r =
            lerp(m_state.start_color.r, m_state.target_color.r);
        m_state.cur_color.g =
            lerp(m_state.start_color.g, m_state.target_color.g);
        m_state.cur_color.b =
            lerp(m_state.start_color.b, m_state.target_color.b);
      }
      else
      {
        auto lerpLin = [&](uint16_t a, uint16_t b) {
          return static_cast<uint16_t>(
              ...
              (static_cast<int32_t>(b) - a) *
                  static_cast<int32_t>(fadeElapsed) /
                  static_cast<int32_t>(m_state.fade_time_ms));
        };
        m_state.cur_color.w =
            lerpLin(m_state.start_color.w, m_state.target_color.w);
...
bool LightEngine::loadStateFromFlash()
{
  ...
  m_state.color_cycle_index =
      clampU8(saved.color_cycle_index, 0, ColorLibrary::kCycleCount - 1);
...
void LightEngine::setFirstCommissionDone()
{
  ...
  NvmStorage::instance().writeData(NVM3_KEY_FIRST_COMMISSION_DONE, &flag,
                                   sizeof(flag));
...
{
  return !m_state.mix_lighting_effects &&
         m_state.effect_mode == EffectModeNone && !m_state.fading;
}

void LightEngine::init()
{
  LOG_LIGHT("[LightEngine] init");
  m_state.is_on              = true;
  ...
  uint8_t flag = 0;
  if (NvmStorage::instance().readData(NVM3_KEY_FIRST_COMMISSION_DONE, &flag,
                                      sizeof(flag)) == ECODE_NVM3_OK &&
  ...
  LOG_LIGHT("[LightEngine] state on=%d bright=%u colorIdx=%u", m_state.is_on,
            m_state.brightness, m_state.color_cycle_index);
...
uint8_t LightEngine::setLowBatteryProtection(bool protect)
{
  ...
  LOG_LIGHT("[LightEngine] low battery protect=%d", protect);
