...
public:
  ...
  void StartSequence(const SequenceStep* steps, uint8_t count,
                     bool isLoopForever = false);

  /* 启动单步灯效（不涉及链式时序） */
  void StartSingleEffect(LightEffectEngine::EffectRenderAction pAction,
                         const uint16_t* targetChannels, uint8_t brightness,
                         uint16_t durationMs);
  /* 强行终止整条时序链并让硬件输出归零 */
  void StopSequence();
