...
LightSequenceScheduler::LightSequenceScheduler()
    : m_totalSteps(0U), m_currentStepIndex(0U), m_currentRepeatLeft(0U),
      m_isSequenceActive(false), m_isLoopForever(false)
{
...
void LightSequenceScheduler::Init()
{
  ...
  LightEffectEngine::Instance().RegisterMixedTimingCallback(
      LightSequenceScheduler::EngineSequenceBridgeRaw);
}
 ...
 */
void LightSequenceScheduler::StartSequence(const SequenceStep* steps,
                                           uint8_t count, bool isLoopForever)
{
  ...
  memcpy(m_stepPool, steps,
         static_cast<size_t>(safeCount) * sizeof(SequenceStep));
 ...
 */
void LightSequenceScheduler::StartSingleEffect(
    LightEffectEngine::EffectRenderAction pAction,
    const uint16_t* targetChannels, uint8_t brightness, uint16_t durationMs)
{
  ...
  for (uint8_t i = 0U; i < LightEffectEngine::Instance().GetActiveChannels();
       ++i)
  {
  ...
  // 4. 全权代客调用底层引擎拉起高频渲染
  LightEffectEngine::Instance().StartEffect(pAction, targetChannels, brightness,
                                            durationMs);
}
...
void LightSequenceScheduler::ExecuteCurrentStep()
{
  ...
  LightEffectEngine::Instance().StartEffect(step.pAction, step.targetChannels,
                                            step.brightness, step.durationMs);
}
...
void LightSequenceScheduler::EngineSequenceBridgeRaw()
{
  ...
  else
  {
    ...
    {
      self.m_currentRepeatLeft =
          self.m_stepPool[self.m_currentStepIndex].repeatCount;
      self.ExecuteCurrentStep();
    }
    else
    {
