...
IadcHal::IadcHal(GPIO_Port_TypeDef port, unsigned int pin)
...
void IadcHal::AllocateAnalogBus(void)
{
  if (m_port == gpioPortA)
  {
    if (0 == m_pin % 2)
...
void IadcHal::CommonConfig(IADC_Init_t& init, IADC_AllConfigs_t& allConfigs)
{
...
void IadcHal::Init(void)
{
  ...
  // 4.
  // 复位并初始化结构体（仅在全局第一次初始化时进行，避免破坏已有的其他通道初始化）
...
void IadcHal::FreeAnalogBus(void)
{
  if (m_port == gpioPortA)
  {
    if (0 == m_pin % 2)
...
void IadcHal::DeInit(GPIO_Mode_TypeDef newMode, unsigned int outValue)
{
...
int32_t IadcHal::ReadRawResult(void)
{
...
uint32_t IadcHal::ReadVoltageMilliVolts(void)
{
  int32_t rawData = ReadRawResult();
  if (rawData < 0)
    return 0;
  // return ((uint32_t)rawData * m_vRefMv) / 4095;
  return ((uint32_t)rawData * m_vRefMv) >>
         12; // 4095 约等于 2^12，使用位移代替除法以提高效率
}

uint32_t IadcHal::ReadChipAvddMilliVolts(void)
{
