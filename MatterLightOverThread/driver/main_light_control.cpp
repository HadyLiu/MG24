...
// 13种颜色档位（放大10倍，避免浮点数，1023代表100.0%）
typedef struct
{
    uint16_t w; // 0 ~ 1023 (对应0%~100%)
    uint16_t r;
    uint16_t g;
    uint16_t b;
} Color_Fixed_t;
...
// ============================================================================
static const uint16_t CUBIC_BREATH_TABLE[64] = {0,    1,    4,    9,    16,   25,   36,   49,   64,   81,   100,  121,  144,  169,  196,  225,
...
{
    if (w > PWM_MAX_RESOLUTION)
        w = PWM_MAX_RESOLUTION;
    if (r > PWM_MAX_RESOLUTION)
        r = PWM_MAX_RESOLUTION;
    if (g > PWM_MAX_RESOLUTION)
        g = PWM_MAX_RESOLUTION;
    if (b > PWM_MAX_RESOLUTION)
        b = PWM_MAX_RESOLUTION;
 ...
 */
static void Calculate_Target_PWM(uint8_t brightness_pct, uint8_t color_idx, uint32_t *out_w, uint32_t *out_r, uint32_t *out_g, uint32_t *out_b)
{
    if (brightness_pct == 0)
    {
...
void Light_Logic_Tick_10ms(void)
{
    ...
    if (g_priority_state != Lamp_Priority_State_t::LAMP_STATE_NORMAL)
    {
        switch (g_priority_state)
        {
        case Lamp_Priority_State_t::LAMP_STATE_FACTORY_RESET:
        ...
        case Lamp_Priority_State_t::LAMP_STATE_CRITICAL_BATTERY:
            Light_Hardware_Output_Raw(0, 0, 0, 0);
            Set_Battery_Power_Enable(false);
            return;

        case Lamp_Priority_State_t::LAMP_STATE_IDENTIFY:
            if ((priority_timer % 800) < 400)
            {
        ...
        case Lamp_Priority_State_t::LAMP_STATE_CHARGE_ERROR: Light_Hardware_Output_Raw(0, 0, 0, 0); return;

        case Lamp_Priority_State_t::LAMP_STATE_LOW_BATTERY_ALERT:
        {
            ...
            Light_Hardware_Output_Raw((g_light.target_w * factor_10bit) >> 10, (g_light.target_r * factor_10bit) >> 10,
                                      (g_light.target_g * factor_10bit) >> 10, (g_light.target_b * factor_10bit) >> 10);
            return;
        }
        }
    }
...
void Light_System_Init(void)
{
    ...
    Calculate_Target_PWM(g_light.saved_brightness, g_light.saved_color_idx, &g_light.target_w, &g_light.target_r, &g_light.target_g,
                         &g_light.target_b);
...
{
    if (g_priority_state == Lamp_Priority_State_t::LAMP_STATE_CRITICAL_BATTERY)
        return;
    ...
    else
    {
        ...
        else
        {
            ...
            Calculate_Target_PWM(0, g_light.saved_color_idx, &g_light.target_w, &g_light.target_r, &g_light.target_g, &g_light.target_b);
...
{
    if (!g_light.is_on || g_priority_state == LAMP_STATE_CRITICAL_BATTERY)
        return;
...
{
    if (color_idx >= TOTAL_COLOR_MODES)
        color_idx = 2;
...
void Light_Set_Priority_State(Lamp_Priority_State_t new_state)
{
    ...
    if (g_priority_state == LAMP_STATE_NORMAL)
    {
        ...
        Calculate_Target_PWM(g_light.is_on ? g_light.saved_brightness : 0, g_light.saved_color_idx, &g_light.target_w, &g_light.target_r,
                             &g_light.target_g, &g_light.target_b);
