...
void power_Wire(void)
{
  ...
  PowerServer::Instance().RegisterBatteryVoltHandler([](BatteryVoltLevel level) {
    LightDecisionCenter::Instance().ProcessBatteryVoltLevel(level);
  });
  ...
  LightDecisionCenter::Instance().RegisterBatteryWarnIndicatorCallback(
      EntryBatteryWarnBlink);

  PowerServer::Instance().OnLightOutputChanged(
      LightEffectEngine::Instance().IsAnyChannelActive());
}
...
void button_Init(void)
{
  ...
  ButtonService::Instance().RegisterKeyEventHandler([](KeyEventType event) {
    if (event == KeyEventType::LongPressClearNet)
    {
      ...
      // 调用执行函数 重置网络
      MatterBridge::Instance().MatterExecuteCmd(
          MatterExecuteElement::kClearNetwork);
    }
    // 按键时间事件投递至 LDC
